...
function CustomerPinConfirmation() {
  ...
  const { isLoading: isLoadingCreateOrder, mutate: createOrder } = useCreateOrder(
    (response) => {
    ...
    (error) => {
      setErrorMessage(error?.message ?? 'Pembelian gagal coba lagi nanti')
    }
  )

  function handleSubmitPin() {
    const payload = JSON.parse(localStorage.payloadorder)
    createOrder({
      ...payload,
              ...
              <div className="bg-white rounded-md shadow-custom pb-2">
                <div className="px-4 pt-6">
                  <div className="flex justify-between items-center">
                    <h4 className="text-lg -mt-0.5 font-medium">Konfirmasi PIN Transaksi Baru</h4>
                  </div>
                ...
                <Card className="w-full shadow-none">
                  ...
                  <Button 
                    ...
                    type={isLoadingCreateOrder ? 'processing' : (pin1 && pin2 && pin3 && pin4 && pin5 && pin6) ? 'submit' : 'disabled'}
                    bg={isLoadingCreateOrder ? 'bg-gray-300' : (pin1 && pin2 && pin3 && pin4 && pin5 && pin6) ? 'bg-wi-blue' : 'bg-gray-300'}
