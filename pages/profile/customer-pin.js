...
function CustomerPin() {
  ...
  const {isLoading, mutate, error} = useMutation('check-pin', (pin) => fetchAuthPost(`users/check-pin`, {pin:pin}), {
    onSuccess() {
      localStorage.old_pin = `${pin1}${pin2}${pin3}${pin4}${pin5}${pin6}`
      router.push('/profile/customer-new-pin')
    },
  })
  // function handleSubmitPin() {
  //   localStorage.old_pin = `${pin1}${pin2}${pin3}${pin4}${pin5}${pin6}`
    // router.push('/profile/customer-pin-confirmation')
  //   router.push('/profile/customer-new-pin')
  // }
              ...
              <div className="bg-white rounded-md shadow-custom pb-2">
                <div className="px-4 pt-6">
                  <div className="flex justify-between items-center">
                    <h4 className="text-lg -mt-0.5 font-medium">Masukan PIN Transaksi Lama</h4>
                  </div>
                ...
                <Card className="w-full shadow-none">
                  {
                    error?.message && (
                      <div className='w-full flex justify-center pb-2'>
                        <span className='text-red-500 text-xs text-center'>{error?.message}</span>
                      </div>
                    )
                  }
                  ...
                  <div className='flex flex-row justify-center items-center my-4'>
                    ...
                    <Link href="/reset-pin"><span className="text-sm text-wi-blue cursor-pointer">Lupa PIN Transaksi</span></Link>
                  </div>
                  <Button 
                    className="w-full font-normal text-sm mt-8" 
                    type={(pin1 && pin2 && pin3 && pin4 && pin5 && pin6) ? (!isLoading ? 'submit' : 'disabled') : 'disabled'}
                    bg={(pin1 && pin2 && pin3 && pin4 && pin5 && pin6) ? 'bg-wi-blue' : 'bg-gray-300'}
                    onClick={() => mutate(`${pin1}${pin2}${pin3}${pin4}${pin5}${pin6}`)}
                  >
                    Berikutnya
                  </Button>
                </Card>
