...
function CustomerPinConfirmation() {
  ...
  const { isLoading, mutate, error } = useMutation((pin) => fetchAuthPost(`users/forgot-pin/${router.query.token}`, pin), {
    onSuccess() {
      router.push('/profile')
    },
    onError() {
      setIsDisabled(true)
      setErrorMessage('Ubah PIN transaksi gagal')
    },
  })
  ...
  function handleSubmitPin() {
    if (localStorage.pin !== `${pin1}${pin2}${pin3}${pin4}${pin5}${pin6}`) {
      ...
      mutate({
        new_pin:localStorage.pin,
        confirmation_pin:`${pin1}${pin2}${pin3}${pin4}${pin5}${pin6}`
      })
    }
  }
