...
export default function ResetPin() {
  ...
  const {mutate, isLoading} = useMutation(
    ...
    () => fetchPost('users/forgot-pin', {email}),
    {
      onSuccess() {
        setSuccess(true)
        setTimeout(() => {
      ...
      <section className="flex flex-col h-screen">
        ...
        <section className="max-w-4xl w-full self-center flex-1 flex items-center mb-10 z-10">
          <section className="p-4 sm:p-0 grid grid-cols-1 sm:grid-cols-2 gap-4 w-full">
            ...
            <div>
              <h4 className="text-dnr-turqoise text-lg  mb-2">Atur Ulang PIN</h4>
              <p className="text-xs text-left text-gray-500 mb-4">
                Masukkan e-mail yang terdaftar. Kami akan mengirimkan link untuk atur ulang PIN.
              </p>
                ...
                <form className="space-y-4" onSubmit={onSubmit}>
                  ...
                  {success ? (
                    <p className="text-sm text-gray-500">
                      Silahkan Periksa email anda untuk mendapatkan akses reset PIN
                    </p>
                  ) : null}
